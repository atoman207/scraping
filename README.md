...
| 3-3 1688 | `Alibaba1688Sourcing` — キーワード検索・画像検索のディープリンク生成のみ | ログイン必須のため下記参照 |
...
一致度が40%に届く候補が無ければ、上位5件の中から安いものを選びます。

**1688 について**: 検索ページがログイン必須(未ログインだと `login.taobao.com` に飛ばされます)で、
ログインを突破する実装は入れていません。そのため**キーワード検索と画像検索のURLを組み立てて出す**
だけにしています。リンクを開いてログインすれば、そのまま検索結果が見られます。
